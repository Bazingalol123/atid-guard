...
public class AuthRepository
{
    ...
    public async Task<AdminResults> DeleteUser(int userId)
    {
        ...
        var user = await GetUserById<UserForAdmin>(userId);
        if (user == null)
            return new AdminResults { Result = AuthResults.UserNotFound };

        try
        {
            // Start a transaction-like approach by deleting roles first, then user
            // Delete user roles first (foreign key constraint)
            string deleteRolesQuery = "DELETE FROM UserRoles WHERE UserId = @UserId";
            await _db.SaveDataAsync(deleteRolesQuery, new { UserId = userId });

            // Delete the user
            string deleteUserQuery = "DELETE FROM Users WHERE Id = @Id";
            int rowsAffected = await _db.SaveDataAsync(deleteUserQuery, new { Id = userId });

            if (rowsAffected > 0)
            {
                return new AdminResults { Result = AuthResults.Success, User = user };
            }
            else
            {
